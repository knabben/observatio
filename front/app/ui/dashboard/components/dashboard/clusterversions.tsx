...
const useComponentVersions = () => {
  const {data: component, isLoading, error} = useFetchState<Component[]>(
...
export default function ClusterVersions() {
  ...
  return (
    <Card shadow="md" className={roboto.className} radius="md" withBorder>
      ...
      {!isLoading && !error && component.length === 0 && <EmptyState label="No components found"/>}
      {!isLoading && !error && component.length > 0 && <VersionsTable components={component}/>}
    </Card>
  );
}
